...
Se as tabelas já existirem, mas a importação retornar que a função `importar_performance_mes` não foi encontrada no cache, execute somente `corrigir_rpc_importar_performance.sql` e recarregue a página.
