...
(function () {
  ...
  const TEMPO_LIMITE_AUTH_MS = Number(window.OPGEST_AUTH_TIMEOUT_MS) || 8000;
  ...
  function aguardarAuth(operacao, descricao) {
    let timeoutId;
    const timeout = new Promise((_, rejeitar) => {
      timeoutId = setTimeout(() => rejeitar(new Error(`Tempo limite excedido ao ${descricao}.`)), TEMPO_LIMITE_AUTH_MS);
    });
  ...
  async function loginComGoogle() {
    ...
    try {
      const { error } = await aguardarAuth(db.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: window.location.origin + window.location.pathname
        }
      }), 'abrir o login do Google');
      if (error) throw error;
    } catch (e) {
      setAuthLoading(false);
      setAuthMessage('Erro ao autenticar com Google: ' + e.message, 'error');
    }
  }
  ...
  async function inicializar() {
    ...
    try {
      const { data, error } = await aguardarAuth(db.auth.getSession(), 'validar a sessao');
      if (error) throw error;
      return await validarSessao(data.session);
    } catch (e) {
      console.error(e);
      Permissions.limparContexto();
      mostrarLogin('Nao foi possivel validar a sessao automaticamente. Clique em Entrar com Google.');
      return false;
    } finally {
      setAuthLoading(false);
    }
