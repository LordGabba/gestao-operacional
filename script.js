...
async function carregarPerformanceOperacional() {
  ...
  if (cards) cards.innerHTML = '<div class="loading-inline"><div class="spinner"></div> Carregando...</div>';
  ...
  try {
    ...
    const mesPadrao = APP.filtros.performance.competencia_mes || meses[0] || new Date().toISOString().slice(0, 7);

    APP.filtros.performance.competencia_mes = mesPadrao;
    if (filtroMes) filtroMes.value = mesPadrao;
    if (competenciaInput && !competenciaInput.value) competenciaInput.value = mesPadrao;

    APP.dados.performance = await DB.performance.listar({ competencia_mes: mesPadrao });
    preencherFiltrosPerformance(meses);
    configurarEventosPerformance();
    renderizarPerformanceOperacional();
  } catch (e) {
    if (cards) cards.innerHTML = `<div class="empty-state"><div class="empty-title">Erro ao carregar performance</div><div class="empty-desc">${escapeHtml(e.message)}</div></div>`;
    toast('Erro ao carregar Performance Operacional: ' + e.message, 'error');
  }
}
...
function preencherFiltrosPerformance(mesesImportados = []) {
  const dados = APP.dados.performance || {};
  ...
  const registros = [...(dados.mop || []), ...(dados.atendimento || []), ...(dados.csat || [])];
  ...
  if (liderEl) {
    ...
    liderEl.value = filtros.lider || '';
  }

  const liderSelecionado = liderEl?.value || filtros.lider || '';
  const pessoas = new Map();
  registros
    .filter(item => !liderSelecionado || item.lider === liderSelecionado)
    .forEach(item => {
      const nome = item.colaborador_nome || item.agent_name;
      if (nome) pessoas.set(nome, nome);
    });

  if (colaboradorEl) {
    colaboradorEl.innerHTML = '<option value="">Todos</option>' + [...pessoas.keys()].sort().map(nome =>
      `<option value="${escapeHtml(nome)}">${escapeHtml(nome)}</option>`
    ).join('');
    colaboradorEl.value = pessoas.has(filtros.colaborador_nome) ? filtros.colaborador_nome : '';
    APP.filtros.performance.colaborador_nome = colaboradorEl.value;
  }
}
...
async function aplicarFiltrosPerformance() {
  ...
  APP.filtros.performance = {
    ...
    colaborador_nome: document.getElementById('performance-filtro-colaborador')?.value || ''
  };

  if (APP.filtros.performance.competencia_mes !== mesAnterior) {
    APP.dados.performance = await DB.performance.listar({ competencia_mes: APP.filtros.performance.competencia_mes });
  }

  preencherFiltrosPerformance(await DB.performance.listarMesesImportados());
  renderizarPerformanceOperacional();
}
...
function dadosFiltradosPerformance() {
  ...
  const dentroDoPeriodo = item => {
    ...
    if (filtros.data_inicio && data && data < filtros.data_inicio) return false;
    if (filtros.data_fim && data && data > filtros.data_fim) return false;
    if (filtros.lider && item.lider !== filtros.lider) return false;
    const nome = item.colaborador_nome || item.agent_name;
    if (filtros.colaborador_nome && nome !== filtros.colaborador_nome) return false;
    return true;
  };

  return {
    mop: (APP.dados.performance.mop || []).filter(item => {
      if (filtros.lider && item.lider !== filtros.lider) return false;
      if (filtros.colaborador_nome && item.colaborador_nome !== filtros.colaborador_nome) return false;
      return true;
    }),
    atendimento: (APP.dados.performance.atendimento || []).filter(dentroDoPeriodo),
    csat: (APP.dados.performance.csat || []).filter(dentroDoPeriodo),
    agentHistory: (APP.dados.performance.agentHistory || []).filter(dentroDoPeriodo)
  };
}
...
function calcularMediaPonderada(registros, campo) {
  ...
  (registros || []).forEach(item => {
    ...
    const valor = campo === 'tme_segundos'
      ? Number(item.tme_total_segundos ?? item.tme_segundos)
      : Number(item[campo]);
...
function calcularCsat(registros) {
  const notas = (registros || []).map(item => Number(item.nota)).filter(Number.isFinite);
...
function classificarMetaPerformance(valor, meta, maiorMelhor = false) {
  ...
  if (maiorMelhor) {
    if (valor < meta) return 'fora';
    return valor < meta * 1.06 ? 'atencao' : 'dentro';
  }
  if (valor > meta) return 'fora';
  return valor > meta * 0.9 ? 'atencao' : 'dentro';
}
...
function renderizarCardsPerformance(dados = dadosFiltradosPerformance()) {
  ...
  const totalTickets = dados.agentHistory.length || ticketsFinalizados;
  const filas = [
    ...dados.agentHistory.map(item => Number(item.queue_time_segundos)),
    ...atendimento.map(item => Number(item.tmaxe_segundos ?? item.tme_total_segundos ?? item.tme_segundos))
  ].filter(Number.isFinite);
  const cards = [
    { nome: 'Total de Tickets', valor: totalTickets, formato: 'numero', status: 'info' },
    { nome: 'Tickets Finalizados', valor: ticketsFinalizados, formato: 'numero', status: 'info' },
    { nome: 'TMAX.E', valor: filas.length ? Math.max(...filas) : null, formato: 'tempo', status: 'info' },
  ...
  el.innerHTML = cards.map(card => {
    ...
    const valor = !temValor ? '-' : card.formato === 'tempo'
      ...
      : card.formato === 'percentual' ? `${card.valor.toFixed(1)}%` : Number(card.valor).toLocaleString('pt-BR');
    const meta = card.meta === undefined ? 'Informativo' : card.formato === 'tempo'
      ? formatarSegundosParaHHMMSS(card.meta) : `${card.meta}%`;
    const diferenca = card.meta === undefined || !temValor ? '-' : card.formato === 'tempo'
      ? formatarDiferencaTempoPerformance(card.valor - card.meta)
      : `${card.valor - card.meta >= 0 ? '+' : ''}${(card.valor - card.meta).toFixed(1)} p.p.`;
    const proporcao = card.meta && temValor ? Math.min(100, (card.valor / card.meta) * 100) : 100;
    const rotulos = { dentro: 'Dentro da meta', fora: 'Fora da meta', atencao: 'Atenção', info: 'Informativo', 'sem-dados': 'Sem dados' };
    return `
      <div class="performance-metric-card status-${status}">
        ...
        <div class="performance-progress"><span style="width:${proporcao}%"></span></div>
...
function renderizarGraficoCsatPerformance(dados = dadosFiltradosPerformance()) {
  ...
  const porDia = Object.entries(grupos).map(([data, itens]) => ({ chave: data, valor: calcularCsat(itens) || 0 }));
  if (!porDia.length) {
  ...
  el.innerHTML = '<div class="performance-csat-wrap"><div class="performance-csat-target">Meta 85%</div><div class="performance-chart-bars">' +
    porDia.map(item => `
      <div class="performance-chart-column">
        <div class="performance-chart-value">${item.valor.toFixed(1)}%</div>
        <div class="performance-chart-bar csat" style="height:${Math.max(3, item.valor)}%"></div>
...
function renderizarTabelaRankingPerformance(dados = dadosFiltradosPerformance()) {
  ...
  tbody.innerHTML = ranking.length ? ranking.map((item, indice) => `
    <tr>
      ...
      <td>${item.csat === null ? '-' : item.csat.toFixed(1) + '%'}</td>
      <td><span class="badge badge-${item.status === 'Dentro da meta' ? 'green' : item.status === 'Atenção' ? 'yellow' : 'red'}">${item.status}</span></td>
...
function montarRankingPerformance(dados = dadosFiltradosPerformance()) {
  const grupos = agruparPerformance(dados.atendimento, item =>
    item.colaborador_email || item.usuario_blip || item.colaborador_nome || 'Sem identificação'
  );
  const csatGrupos = agruparPerformance(dados.csat, item =>
    item.colaborador_email || item.usuario_blip || item.colaborador_nome || 'Sem identificação'
  );
  ...
  return Object.entries(grupos).map(([chave, itens]) => {
    ...
    const csat = calcularCsat(csatGrupos[chave] || []);
    const estados = [
    ...
    return {
      ...
      status: estados.includes('fora') ? 'Fora da meta' : estados.includes('atencao') ? 'Atenção' : 'Dentro da meta'
    };
...
async function previewImportacaoPerformance(tipo, arquivo) {
  ...
  if (!competencia) {
    toast('Selecione o mês de competência antes do arquivo.', 'warning');
    return;
  }
  ...
  try {
    ...
    const mopAtual = tipo === 'mop'
      ? linhas.map(row => mapearRegistroPerformance('mop', row, competencia).registro)
      : obterMopImportacaoPerformance();
    const processados = linhas.map((row, indice) => mapearRegistroPerformance(tipo, row, competencia, mopAtual, indice + 2));
  ...
  el.innerHTML = `
    <div class="performance-preview-card">
      ...
      <div class="mt-4 flex gap-2">
        <button class="btn btn-primary" onclick="confirmarImportacaoPerformance()">Confirmar importação</button>
...
async function confirmarImportacaoPerformance() {
  ...
  if (!competencia || !ordem.length) {
    toast('Selecione a competência e ao menos um arquivo.', 'warning');
    return;
  }

  try {
    toast('Importando performance do mês...', 'info');
    ...
    APP.filtros.performance.competencia_mes = competencia;
...
function mapearRegistroPerformance(tipo, row, competencia, mop = [], linha = 0) {
  const nome = valorPerformance(row, ['Atendente', 'AgentName', 'Nome do Agente', 'Colaborador', 'Nome']);
  const email = valorPerformance(row, ['E-mail', 'Email', 'AgentEmail']);
  const usuarioBlip = valorPerformance(row, ['Usuário Blip', 'Usuario Blip', 'AgentIdentity', 'Identity']);
  const data = normalizarData(valorPerformance(row, ['Data', 'StorageDate', 'CloseDate', 'Data da avaliação', 'Data da avaliacao']));
  const base = {
    competencia_mes: competencia,
  ...
  if (tipo === 'mop') {
    registro = {
      competencia_mes: competencia,
      colaborador_nome: String(nome || valorPerformance(row, ['Nome do colaborador'])).trim(),
      colaborador_email: String(email || '').trim().toLowerCase(),
      usuario_blip: String(usuarioBlip || '').trim(),
      lider: String(valorPerformance(row, ['Líder', 'Lider', 'Reporte', 'Supervisor']) || '').trim(),
      status: String(valorPerformance(row, ['Status']) || 'Ativo').trim(),
      origem_arquivo: tipo
    };
    if (!registro.colaborador_nome && !registro.colaborador_email && !registro.usuario_blip) {
  ...
  if (tipo === 'metricas') {
    ...
    registro = {
      ...base,
      sequential_id: String(valorPerformance(row, ['SequentialId', 'Sequential ID', 'Ticket']) || '').trim(),
      ...
      close_date: normalizarData(valorPerformance(row, ['CloseDate', 'Data de fechamento']))
    };
  } else {
    registro = {
      ...base,
      nota: normalizarNumeroPerformance(valorPerformance(row, ['Nota', 'Avaliação', 'Avaliacao', 'Rating'])),
      avaliacao_id: String(valorPerformance(row, ['ID', 'Id', 'Avaliação ID', 'Avaliacao ID']) || '').trim()
    };
  }

  if (!registro.data_referencia) inconsistencias.push(`Linha ${linha}: data inválida ou ausente`);
  if (!base.colaborador_nome && !base.colaborador_email && !base.usuario_blip) inconsistencias.push(`Linha ${linha}: colaborador sem identificação`);
  const relacionado = relacionarComMop(registro, mop);
  if (!relacionado.encontrado) inconsistencias.push(`${base.colaborador_nome || base.colaborador_email || base.usuario_blip || `Linha ${linha}`} não encontrado na MOP`);
...
function relacionarComMop(registro, mop = []) {
  ...
  const encontrado = (blip && (mop || []).find(item => normalizarTextoPerformance(item.usuario_blip) === blip))
...
function normalizarNumeroPerformance(valor) {
  ...
  const texto = String(valor).trim().replace(/\./g, '').replace(',', '.').replace(/[^\d.-]/g, '');
