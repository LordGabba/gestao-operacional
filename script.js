...
async function carregarPerformanceOperacional() {
  ...
  atualizarInterfaceTipoImportacaoPerformance(false);
  ...
  carregarHistoricoImportacoesPerformance();
}
...
async function carregarHistoricoImportacoesPerformance() {
  if (!Security.verificarPermissao?.('importar_performance_operacional') || typeof DB.performance.listarImportacoes !== 'function') return;
  try {
    renderizarHistoricoImportacoesPerformance(await DB.performance.listarImportacoes(20));
  } catch (e) {
    console.warn('HistÃ³rico de importaÃ§Ãµes nÃ£o pÃ´de ser carregado:', e.message);
  }
}

function renderizarHistoricoImportacoesPerformance(registros = []) {
  const tbody = document.getElementById('performance-import-history-body');
  if (!tbody) return;
  tbody.innerHTML = registros.length ? registros.map(item => `
    <tr>
      <td>${escapeHtml(item.competencia_data || item.competencia_mes || '-')}</td>
      <td>${escapeHtml(`${rotuloTipoPerformance(item.tipo_arquivo)} · ${item.tipo_importacao || (item.competencia_data ? 'DIARIA' : 'MENSAL')}`)}</td>
      <td>${escapeHtml(item.nome_arquivo || '-')}</td>
      <td>${Number(item.total_registros || 0).toLocaleString('pt-BR')}</td>
      <td><span class="badge ${item.status === 'sucesso' ? 'badge-green' : 'badge-red'}">${escapeHtml(item.status || '-')}</span></td>
      <td>${escapeHtml(formatarDataHoraPerformance(item.importado_em))}</td>
    </tr>`).join('') : '<tr><td colspan="6" class="text-muted text-center">Nenhuma importaÃ§Ã£o registrada</td></tr>';
}
...
function calcularCsat(registros) {
  ...
  let positivas = 0;
  (registros || []).forEach(item => {
    if (item.total_avaliacoes !== undefined && item.total_avaliacoes !== null) {
      total += Number(item.total_avaliacoes) || 0;
      positivas += Number(item.avaliacoes_positivas) || 0;
      return;
    }
    const nota = normalizarNotaPerformance(item.nota);
    if (nota === null) return;
    total += 1;
    if (nota >= 4 && nota <= 5) positivas += 1;
  });
  return total ? (positivas / total) * 100 : null;
}
...
function renderizarGraficoCsatPerformance(dados = dadosFiltradosPerformance()) {
  ...
  if (!porDia.length) {
    el.innerHTML = '<div class="empty-state"><div class="empty-title">Sem avaliações no período selecionado</div></div>';
    return;
  }
...
function renderizarTabelaRankingPerformance(dados = dadosFiltradosPerformance()) {
  ...
  tbody.innerHTML = ranking.length ? ranking.map((item, indice) => `
    <tr>
      ...
      <td>${escapeHtml(item.usuarioBlip || '-')}</td>
      ...
      <td>${formatarSegundosParaHHMMSS(PERFORMANCE_METAS.tme_segundos)}</td>
      <td>${formatarSegundosParaHHMMSS(item.tm1r)}</td>
      <td>${formatarSegundosParaHHMMSS(PERFORMANCE_METAS.tm1r_segundos)}</td>
      <td>${formatarSegundosParaHHMMSS(item.tmr)}</td>
      <td>${formatarSegundosParaHHMMSS(PERFORMANCE_METAS.tmr_segundos)}</td>
      <td>${formatarSegundosParaHHMMSS(item.tma)}</td>
      <td>${formatarSegundosParaHHMMSS(PERFORMANCE_METAS.tma_segundos)}</td>
      <td>${item.csat === null ? '-' : formatarPercentualPerformance(item.csat)}</td>
      <td>85%</td>
      <td><span class="badge ${classeStatusGeralPerformance(item.status)}">${item.status}</span></td>
    </tr>`).join('') : '<tr><td colspan="17" class="text-muted text-center">Nenhum colaborador encontrado</td></tr>';
}
...
function montarRankingPerformance(dados = dadosFiltradosPerformance()) {
  ...
  return Object.entries(grupos).map(([chave, itens]) => {
    ...
    const totalComDados = estados.filter(estado => estado !== 'sem-dados').length;
    const totalFora = estados.filter(estado => estado === 'fora').length;
    return {
      nome: itens[0].colaborador_nome || chave,
      usuarioBlip: itens[0].usuario_blip || '',
      lider: itens[0].lider || '',
      tickets,
      produtividade: tickets / dias,
      tme, tm1r, tmr, tma, csat,
      status: !totalComDados ? 'Sem dados' : totalFora >= 3 ? 'Crítico' : totalFora ? 'Atenção' : 'Saudável'
    };
...
function renderizarPontosAtencaoPerformance(dados = dadosFiltradosPerformance()) {
  ...
  const foraTm1r = ranking.filter(item => item.tm1r !== null && item.tm1r > PERFORMANCE_METAS.tm1r_segundos).length;
  const foraTmr = ranking.filter(item => item.tmr !== null && item.tmr > PERFORMANCE_METAS.tmr_segundos).length;
  ...
  const maiorTma = [...ranking].filter(item => item.tma !== null).sort((a, b) => b.tma - a.tma)[0];
  const menorCsat = [...ranking].filter(item => item.csat !== null).sort((a, b) => a.csat - b.csat)[0];
  const ofensoresPorLider = {};
  ranking.filter(item => item.status === 'Atenção' || item.status === 'Crítico').forEach(item => {
  ...
  const itens = [
    ...
    ['Fora da meta de TM1.R', foraTm1r],
    ['Fora da meta de TMR', foraTmr],
    ...
    ['Maior TMA', maiorTma ? `${maiorTma.nome} · ${formatarSegundosParaHHMMSS(maiorTma.tma)}` : '-'],
    ...
    ['Colaboradores críticos', ranking.filter(item => item.status === 'Crítico').length],
    ['Colaboradores saudáveis', ranking.filter(item => item.status === 'Saudável').length]
  ];
...
function obterConfiguracaoImportacaoPerformance() {
  const tipoImportacao = document.getElementById('performance-import-tipo')?.value === 'MENSAL' ? 'MENSAL' : 'DIARIA';
  const competencia = document.getElementById('performance-import-competencia')?.value || '';
  const competenciaMes = /^\d{4}-\d{2}/.test(competencia) ? competencia.slice(0, 7) : '';
  const competenciaData = tipoImportacao === 'DIARIA' && /^\d{4}-\d{2}-\d{2}$/.test(competencia) ? competencia : '';
  return { tipoImportacao, competencia, competenciaMes, competenciaData };
}

function atualizarInterfaceTipoImportacaoPerformance(limparPreview = true) {
  const input = document.getElementById('performance-import-competencia');
  const label = document.getElementById('performance-import-competencia-label');
  const ajuda = document.getElementById('performance-import-ajuda');
  if (!input) return;
  const mensal = document.getElementById('performance-import-tipo')?.value === 'MENSAL';
  const valorAnterior = input.value;
  input.type = mensal ? 'month' : 'date';
  input.value = mensal ? valorAnterior.slice(0, 7) : (/^\d{4}-\d{2}-\d{2}$/.test(valorAnterior) ? valorAnterior : `${valorAnterior || new Date().toISOString().slice(0, 7)}-01`);
  if (label) label.textContent = mensal ? 'Mês de competência' : 'Data de referência';
  if (ajuda) ajuda.textContent = mensal
    ? 'O arquivo mensal deve possuir uma coluna de data válida. A importação substitui somente o mês e o tipo selecionados.'
    : 'Métricas, AgentHistory e CSAT substituem somente o dia escolhido. Quando Métricas não possui coluna de data, o calendário define o dia de referência. A MOP continua mensal.';
  if (limparPreview) cancelarImportacaoPerformance();
}
...
async function previewImportacaoPerformance(tipo, arquivo) {
  ...
  const { tipoImportacao, competenciaMes, competenciaData } = obterConfiguracaoImportacaoPerformance();
  if (!competenciaMes || (tipoImportacao === 'DIARIA' && !competenciaData)) {
    toast(`Selecione ${tipoImportacao === 'DIARIA' ? 'o dia' : 'o mês'} da competência antes do arquivo.`, 'warning');
    return;
  }
  ...
  try {
    ...
    const processados = linhas.map((row, indice) => {
      ...
      if (tipo !== 'mop' && tipoImportacao === 'DIARIA' && processado.registro.data_referencia !== competenciaData) {
        processado.valido = false;
        processado.inconsistencias.push(`Linha ${indice + 2}: data fora da competência diária selecionada`);
      }
      if (tipo !== 'mop' && tipoImportacao === 'MENSAL' && processado.registro.data_referencia?.slice(0, 7) !== competenciaMes) {
        processado.valido = false;
        processado.inconsistencias.push(`Linha ${indice + 2}: data inválida ou fora da competência mensal selecionada`);
      }
      return processado;
    });
    const totalAnteriores = typeof DB.performance.contarRegistrosSubstituidos === 'function'
      ? await DB.performance.contarRegistrosSubstituidos(tipo, competenciaMes, competenciaData, tipo === 'mop' ? 'MENSAL' : tipoImportacao).catch(() => null)
      : null;
    window._performanceImportacao.arquivos[tipo] = {
      tipo,
      nome: arquivo.name,
      tipoImportacao: tipo === 'mop' ? 'MENSAL' : tipoImportacao,
      competenciaMes,
      competenciaData,
    ...
    if (tipo !== 'mop' && !window._performanceImportacao.arquivos[tipo].registros.length) {
      toast(`Nenhum registro de ${rotuloTipoPerformance(tipo)} corresponde à competência selecionada. Revise a data e o tipo de importação.`, 'warning');
    }
...
function renderizarPreviewImportacaoPerformance() {
  ...
  el.innerHTML = `
    <div class="performance-preview-card">
      ...
      <div class="performance-preview-list">
        ${arquivos.map(item => `
          <div class="performance-preview-line">
            <span>${escapeHtml(rotuloTipoPerformance(item.tipo))} · ${escapeHtml(item.tipoImportacao)} · ${escapeHtml(item.nome)}</span>
            <strong>${item.registros.length} válido(s) de ${item.totalLidos} · substitui ${item.totalAnteriores === null ? '?' : item.totalAnteriores}</strong>
          </div>`).join('')}
      </div>
      ...
      ${inconsistencias.length ? `<div class="performance-inconsistencies">${[...new Set(inconsistencias)].slice(0, 20).map(item => `<div>${escapeHtml(item)}</div>`).join('')}</div>` : ''}
...
async function confirmarImportacaoPerformance() {
  ...
  if (!competenciaMes || !ordem.length) {
    toast('Selecione a competência e ao menos um arquivo.', 'warning');
    return;
  }
  if (ordem.some(tipo => arquivos[tipo].competenciaMes !== competenciaMes || (tipo !== 'mop' && arquivos[tipo].tipoImportacao !== tipoImportacao))) {
    toast('A competência ou o tipo de importação mudou. Selecione novamente os arquivos para atualizar a prévia.', 'warning');
    return;
  }
  ...
  const semMop = ordem.filter(tipo => tipo !== 'mop').some(tipo => arquivos[tipo].inconsistencias.some(item => item.includes('não encontrado na MOP')));
  const totalValidos = ordem.reduce((total, tipo) => total + arquivos[tipo].registros.length, 0);
  const totalSemMop = ordem.reduce((total, tipo) => total + arquivos[tipo].inconsistencias.filter(item => item.includes('não encontrado na MOP')).length, 0);
  if ((semMop && !obterMopImportacaoPerformance().length) || (totalValidos && totalSemMop / totalValidos > 0.2)) {
    const confirmou = typeof window.confirm !== 'function' || window.confirm('Há muitos registros sem correspondência na MOP. Deseja continuar conscientemente com a importação?');
    if (!confirmou) return;
  }

  try {
    toast(`Importando performance ${tipoImportacao === 'DIARIA' ? 'do dia' : 'do mês'}...`, 'info');
    for (const tipo of ordem) {
      const arquivo = arquivos[tipo];
      await DB.performance.importarMes({
        competencia_mes: competenciaMes,
        competencia_data: competenciaData,
        tipo_importacao: arquivo.tipoImportacao,
        tipo_arquivo: tipo,
...
function mapearRegistroPerformance(tipo, row, competencia, mop = [], linha = 0, competenciaData = '') {
  ...
  if (tipo === 'mop') {
    registro = {
      ...
      usuario_blip: String(usuarioBlip || '').trim(),
      matricula: String(matricula || '').trim(),
      user_jira: String(valorPerformance(row, ['User Jira', 'Usuário Jira', 'Usuario Jira', 'Jira']) || '').trim(),
      ...
      reporte: String(valorPerformance(row, ['Reporte', 'Reporta para']) || '').trim(),
      supervisor: String(valorPerformance(row, ['Supervisor', 'Supervisão', 'Supervisao']) || '').trim(),
      celula: String(valorPerformance(row, ['Célula', 'Celula']) || '').trim(),
      grupo: String(valorPerformance(row, ['Grupo', 'Equipe', 'Time']) || '').trim(),
      cargo: String(valorPerformance(row, ['Cargo', 'Função', 'Funcao']) || '').trim(),
  ...
  if (tipo === 'metricas') {
    ...
    registro = {
      ...
      atendimento_segundos: normalizarTempoParaSegundos(valorPerformance(row, ['ServiceTime', 'HandlingTime', 'Tempo de atendimento'])),
      storage_date: String(valorPerformance(row, ['StorageDate', 'Storage Date']) || '').trim() || null,
    ...
    registro = {
      ...
      comentario: String(valorPerformance(row, ['Comentário', 'Comentario', 'Comments', 'Comment']) || '').trim(),
      linha_origem: linha
    };
  }
...
function classeStatusGeralPerformance(status) {
  return { 'Saudável': 'badge-green', 'Atenção': 'badge-yellow', 'Crítico': 'badge-red', 'Sem dados': 'badge-gray' }[status] || 'badge-gray';
}

function formatarDataHoraPerformance(valor) {
  if (!valor) return '-';
