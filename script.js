...
async function previewImportacaoPerformance(tipo, arquivo) {
  ...
  try {
    ...
    const mopAtual = tipo === 'mop'
      ? linhas.map(row => mapearRegistroPerformance('mop', row, competenciaMes).registro)
      : obterMopImportacaoPerformance();
    const processados = linhas.map((row, indice) => {
      const processado = mapearRegistroPerformance(tipo, row, competenciaMes, mopAtual, indice + 2);
