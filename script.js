...
async function carregarPerformanceOperacional() {
  ...
  try {
    ...
    APP.dados.performance = await DB.performance.listar({
      competencia_mes: mesPadrao,
    ...
    carregarAgentHistoryPerformanceEmSegundoPlano(mesPadrao);
...
async function carregarMesesPerformanceEmSegundoPlano(selecionarUltimoImportado = false) {
  ...
  try {
    ...
    if (selecionarUltimoImportado && semDados && mesImportado && mesImportado !== APP.filtros.performance.competencia_mes) {
      ...
      APP.dados.performance = await DB.performance.listar({
        competencia_mes: mesImportado,
        incluir_agent_history: false
      });
      preencherFiltrosPerformance(meses);
      renderizarPerformanceOperacional();
      carregarAgentHistoryPerformanceEmSegundoPlano(mesImportado);
      return;
    }
    preencherFiltrosPerformance(meses);
  } catch (e) {
...
async function aplicarFiltrosPerformance() {
  try {
    ...
    if (APP.filtros.performance.competencia_mes !== mesAnterior) {
      APP.dados.performance = await DB.performance.listar({
        competencia_mes: APP.filtros.performance.competencia_mes,
        incluir_agent_history: false
      });
      carregarAgentHistoryPerformanceEmSegundoPlano(APP.filtros.performance.competencia_mes);
    }
...
function renderizarCardsPerformance(dados = dadosFiltradosPerformance()) {
  ...
  const totalTickets = dados.agentHistory.length ? contarTicketsUnicosPerformance(dados.agentHistory) : ticketsFinalizados;
  const filas = [
...
function chavesIdentidadePerformance(item = {}) {
  const chaves = [
    ...
    const normalizado = normalizarTextoPerformance(valor);
...
function chaveAgrupamentoPerformance(item = {}) {
  return chavesIdentidadePerformance(item)[0] || 'sem-identificacao';
}
