  ...
  performance: {
    async listar(filtros = {}) {
      ...
      const criarQuery = tabela => {
        ...
        if (competencia) query = query.eq('competencia_mes', competencia);
        return query;
      };
      const [mop, atendimento, csat, agentHistory] = await Promise.all([
        criarQuery('performance_mop').order('colaborador_nome'),
        criarQuery('performance_atendimento').order('data_referencia'),
        criarQuery('performance_csat').order('data_referencia'),
        criarQuery('performance_agent_history').order('data_referencia')
      ]);
      [mop, atendimento, csat, agentHistory].forEach(resultado => {
      ...
      return {
        ...
        csat: csat.data || [],
        agentHistory: agentHistory.data || []
      };
    },
    ...
    async importarMes({ competencia_mes, tipo_arquivo, nome_arquivo, registros, observacao = '' }) {
      ...
      const { data, error } = await db.rpc('importar_performance_mes', {
      ...
      if (error?.code === 'PGRST202' || error?.message?.includes('importar_performance_mes')) {
        throw new Error('A RPC importar_performance_mes ainda não está disponível no Supabase. Execute performance_operacional.sql ou corrigir_rpc_importar_performance.sql no SQL Editor e recarregue a página.');
      }
      if (error) throw error;
      return data;
    },
    ...
    async listarMesesImportados() {
      ...
      if (error) throw error;
      return [...new Set((data || []).map(item => item.competencia_mes).filter(Boolean))];
    }
  },
