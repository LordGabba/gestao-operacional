  ...
  performance: {
    ...
    async importarAgentHistoryDireto({ competencia_mes, nome_arquivo, registros, observacao = '' }) {
      ...
      const payload = (registros || [])
        ...
        .filter(registro => registro.chave_colaborador && registro.data_referencia);
...
function sanitizarAgentHistoryPerformance(registro, competenciaMes) {
  ...
  const payload = {
    ...
    colaborador_nome: texto(registro.colaborador_nome),
    colaborador_email: texto(registro.colaborador_email).toLowerCase(),
    usuario_blip: texto(registro.usuario_blip),
    lider: texto(registro.lider),
