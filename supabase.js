...
// ============================================================
// Versao de entrega: performance-20260531-07
...
const DB = {
  ...
  performance: {
    async listar(filtros = {}) {
      ...
      const [mop, atendimento, csat] = await Promise.all([
        listarTabelaPerformancePaginada(
          ...
          competencia,
          ['colaborador_nome', 'id'],
          'id,competencia_mes,chave_colaborador,colaborador_nome,colaborador_email,usuario_blip,lider,status'
        ),
        listarTabelaPerformancePaginada(
    ...
    async importarMes({ competencia_mes, competencia_data, tipo_arquivo, nome_arquivo, registros, observacao = '' }) {
      if (!permissaoLiberada('importar_performance_operacional')) return null;
      const competenciaData = normalizarCompetenciaDataPerformance(competencia_data, competencia_mes, registros);
      ...
      if (tipo_arquivo === 'agent_history') {
        return this.importarAgentHistoryDireto({ competencia_mes: competenciaMes, competencia_data: competenciaData, nome_arquivo, registros, observacao });
      }
      const rpc = 'importar_performance_dia';
      const parametros = {
        p_competencia_data: competenciaData,
        p_tipo_arquivo: tipo_arquivo,
    ...
    async importarAgentHistoryDireto({ competencia_mes, competencia_data, nome_arquivo, registros, observacao = '' }) {
      if (!permissaoLiberada('importar_performance_operacional')) return null;
      const competenciaData = normalizarCompetenciaDataPerformance(competencia_data, competencia_mes, registros);
      if (!competenciaData) throw new Error('Selecione uma data válida para importar o AgentHistory.');
      const payload = (registros || [])
        .map(registro => sanitizarAgentHistoryPerformance(registro, competencia_mes))
        .filter(registro => registro.data_referencia === competenciaData && (registro.agent_identity || registro.agent_email || registro.agent_name));
      if (!payload.length) throw new Error('Nenhum registro válido de AgentHistory para o dia selecionado.');
      ...
      const anteriores = await listarTabelaPerformancePaginada(
        ...
        ['data_referencia', 'id'],
        '*',
        { data_referencia: competenciaData }
      );
      ...
      const { error: deleteError } = await db
        ...
        .delete()
        .eq('competencia_mes', competencia_mes)
        .eq('data_referencia', competenciaData);
      if (deleteError) throw deleteError;
